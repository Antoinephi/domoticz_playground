...
###############################################################################
#
# This script has been modified in order to allow use in code.
#     - main & speedtest functions now accept a string parameter for the server id : server_id, set as None by default
#     - speedtest function will not look for the default best server if server_id is left to None, allowing the CLI script to work properly
#
# To use the speedtest script from another python script  :
#     - import speedtest-cli classes and functions 
#     - and just call the main function (with, or without the server id)
#
###############################################################################
...
#edited : added server_id parameter
def speedtest(server_id):
    """Run the full speedtest.net test"""

    global shutdown_event, source, scheme, ulspeed, dlspeed, best
    ...
    if args.list or args.server or server_id: #edit
    ...
    if server_id: #edit
        best = getBestServer(filter(lambda x: x['id'] == server_id, servers))

    if args.server:
        try:
            best = getBestServer(filter(lambda x: x['id'] == server_id,
                                        servers))
        except IndexError:
    ...
    dlspeed =  ((dlspeed / 1000 / 1000) * args.units[1], args.units[0])
    ...
    ulspeed = ((ulspeed / 1000 / 1000) * args.units[1], args.units[0])
    
    #EDIT
    dlspeedk = round(dlspeed[0] / 8, 2)
    ping = round(best['latency'], 0)
    ulspeedk = round(ulspeed[0] / 8, 2)
    ...
    if args.share and args.mini:
        ...
        print_('Share results: %s://www.speedtest.net/result/%s.png' %
               (scheme, resultid[0]))
    return dlspeedk, ulspeedk #edited : return up/down values


def main(server_id=None):
    try:
        return speedtest(server_id)
        # print(dl)
